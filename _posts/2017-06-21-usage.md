...
[Understand the use of executor.backward](https://github.com/dmlc/mxnet/issues/1388),官方issue里面的一个解释，正确姿势：　　
```python
...
def get_fine_tune_model(symbol, arg_params, num_classes, layer_name='flatten0'):
    ...
    net = mx.sym.Convolution(data=net, kernel=(3, 3), stride=(1, 1), num_filter=512, name='fc0')
    net = mx.symbol.FullyConnected(data=net, num_hidden=4096, name='fc1')
    net = mx.sym.Activation(net, act_type='relu')
    net = mx.sym.Dropout(net, p=0.5)
    net = mx.symbol.FullyConnected(data=net, num_hidden=num_classes, name='fc2')
    lgre = mx.sym.LogisticRegressionOutput(data=net, label=label, name='softmax')
    new_args = dict({k:arg_params[k] for k in arg_params if 'fc' not in k})

    print lgre.infer_shape(data=(1, 3, 224, 224), softmax_label=(1, 17))
    print lgre.list_arguments()
    return (lgre, new_args)
```

与特征提取时做法想用得到截断的symbol,不同之处需要冻结一些层，这里有个坑,就是下面这句:  

```python
new_args = dict({k:arg_params[k] for k in arg_params if 'fc' not in k})
```
这时字符串的判断方法，如`"fc" in "fc_weight"==True`这句是对的,所以可以通过较好的名字前缀过滤掉一些层。　　

### 关于multilabel  

首先是rec数据制作，multilabel如何制作数据集，只要满足格式即可。即每行格式如下：　　

```
index\tlabel0\tlabel1\t...labeln\txxx.jpg
...
```
修改mxnet/tools/im2rec.py中对应部分即可，或者自己生成这种格式的lst文件，然后利用im2rec生成rec数据即可。　　

其次是多标签的最终的损失函数的选择,mxnet中没有binary_classification损失函数，因此有两种解决方式：  

* 对于每个标签利用softmaxoutput输入对应的标签，然后将所有损失相加即可；  
* 第二种就是使用回归LogisticRegressionOutput,对于标签数每个位置进行回归。　　
