...
文章认为，原始的smoothl1损失函数，在x绝对值小于1时，梯度为x的绝对值, 即针对不同大小的损失，梯度均为损失的绝对值，这样在梯度更新时，梯度会主要来自值大的部分，为了均衡这部分，文章对较大值的梯度进行压缩，让它随着值的变化不是剧烈，替换为aln(|x|+1),可以看到，当x的绝对值大是，会比x小很多，而且比较平缓。避免因为边界点的较大梯度更新破坏模型向好的方向发展。整个损失函数如下：
